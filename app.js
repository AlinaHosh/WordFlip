(function () {
  ...
  // Безопасный селектор элементов DOM
  ...
  function getLevelColor(level) { return levelColors[level] || 'var(--card-inner-bg, #e5e7eb)'; }
  ...
  if (saveCredentialsBtn) {
    saveCredentialsBtn.addEventListener('click', () => {
      ...
      if (!gk) { showToast('Заполните Gemini API Key!', 'error'); return; }
  ...
  function initSupabase(url, key) {
    try {
      ...
      supabaseClient.auth.onAuthStateChange((event, session) => {
        if (session && session.user) {
          ...
          currentUser = null;
          if (userDisplayBadge) userDisplayBadge.classList.add('hidden');
    ...
    } catch (e) { console.warn('Supabase offline or bad credentials', e); }
  }
  ...
  if (authBtn) {
    authBtn.addEventListener('click', () => {
      if (currentUser && supabaseClient) {
        ...
        if (authModal) authModal.classList.replace('hidden', 'flex');
      }
    });
  }
  if (closeAuthBtn && authModal) closeAuthBtn.addEventListener('click', () => authModal.classList.replace('flex', 'hidden'));

  if (signUpBtn) {
    signUpBtn.addEventListener('click', async () => {
      if (!supabaseClient) { showToast('Сначала укажите Supabase URL и Key в настройках!', 'error'); return; }
      const email = authEmail ? authEmail.value.trim() : '';
      const password = authPassword ? authPassword.value.trim() : '';
      if (!email || !password) { showToast('Заполните Email и пароль', 'error'); return; }
      const { error } = await supabaseClient.auth.signUp({ email, password });
      if (error) showToast(error.message, 'error');
      else showToast('Регистрация успешна! Проверьте почту или войдите.', 'success');
    });
  }
  ...
  if (signInBtn) {
    signInBtn.addEventListener('click', async () => {
      ...
      if (error) showToast(error.message, 'error');
        ...
        showToast('Успешный вход в облако! ☁️', 'success');
        if (authModal) authModal.classList.replace('flex', 'hidden');
      }
    });
  }

  // --- 4. ГЕНЕРАЦИЯ СЛОВ (ГЛАВНАЯ ФУНКЦИЯ) ---
  async function generateBatchWords() {
    const geminiKey = localStorage.getItem('wf_gemini_key');
    if (!geminiKey) {
      showToast('Укажите Gemini API Key в настройках!', 'error');
      if (credentialsModal) credentialsModal.classList.replace('hidden', 'flex');
      return;
    }
    ...
    const prompt = `Сгенерируй строго массив из 5 случайных слов/выражений на языке ${selectedLang} для уровня ${selectedLevel} по теме "${selectedTopic}". ${exclusionPhrase} ` +
      `Верни ТОЛЬКО валидный JSON-массив без markdown разметки или списков. Формат: ` +
      `[{"word": "word", "translation": "перевод на русский", "transcription": "[transcription]", "category": "${selectedTopic}", "examples": [{"en":"sentence1","ru":"перевод1"}]}]`;
    ...
    try {
      ...
      if (data.error) throw new Error(data.error.message || 'Ошибка сервера Google Gemini');
      ...
      showToast('Подборка из 5 слов готова!', 'success');

    } catch (e) {
      console.error('AI Error:', e);
      showToast('Ошибка ИИ: ' + (e.message || 'Попробуйте еще раз'), 'error');
  ...
  if (prevWordBtn) prevWordBtn.addEventListener('click', () => { if (batchCurrentIndex > 0) { batchCurrentIndex--; displayCurrentBatchWord(); } });
  if (nextWordBtn) nextWordBtn.addEventListener('click', () => { if (batchCurrentIndex < batchWords.length - 1) { batchCurrentIndex++; displayCurrentBatchWord(); } });
  ...
  function renderCard(word) {
    ...
    if ($('#cardExamples')) {
      $('#cardExamples').innerHTML = (word.examples || []).map(ex => `
        <div class="brut-border rounded-lg p-3 text-left mb-2" style="background:var(--card-inner-bg, #f9fafb); color:var(--ink, #000);">
  ...
  const addCustomWordBtn = $('#addCustomWordBtn');
  if (addCustomWordBtn) {
    addCustomWordBtn.addEventListener('click', () => {
      ...
      const newWord = {
        ...
        lang: $('#customLang') ? $('#customLang').value : 'English',
      ...
      if (currentUser) {
        ...
        renderVocab();
        showToast('Слово сохранено локально', 'success');
      }
  ...
  function renderHistory() {
    ...
    historyList.innerHTML = localHistory.map((word, index) => `
      <div class="w-full flex items-center justify-between bg-white brut-border rounded-lg p-2 mb-2" style="background:var(--card-inner-bg, #fff);">
  ...
  function renderVocab() {
    ...
    vocabGrid.innerHTML = filtered.map(word => `
      <div class="brut-border rounded-xl p-4 relative ${word.mastered ? 'mastered-card' : ''}" style="background:var(--card-inner-bg, #fff);">
  ...
  if ($('#settingsBtn') && settingsModal) $('#settingsBtn').addEventListener('click', () => settingsModal.classList.replace('hidden', 'flex'));
  if ($('#closeSettingsBtn') && settingsModal) $('#closeSettingsBtn').addEventListener('click', () => settingsModal.classList.replace('flex', 'hidden'));
  if ($('#clearKeyBtn')) {
    $('#clearKeyBtn').addEventListener('click', () => {
      localStorage.clear();
      showToast('Конфигурация сброшена', 'info');
